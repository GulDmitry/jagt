 ...
 * either — with the console threshold off (see {@link ConsoleLogging}) a jagt that failed to bind its port died
 * in complete silence, leaving a shell prompt and a still-serving OLD instance as the only evidence. That one
 * case is worth naming outright, because "another jagt is already running" is what it almost always means.
 */
