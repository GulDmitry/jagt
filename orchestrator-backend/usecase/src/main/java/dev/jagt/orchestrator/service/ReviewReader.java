 ...
 * it: the metered headless assistant, reading the host through the MCP tools of whoever runs jagt. It is a full
 * model call, and the dominant per-task cost.
 *
 * <p>Metering lives here rather than in the caller so a read costs the same wherever it is asked from: every
 * paid read is booked to the task.
 */
@Component
