...
5. When the task is done and verified: leave the changes uncommitted and set status REVIEW_PENDING with a short summary (10 words max). During review rounds: fix locally (still no commit), write draft replies to `review_replies.md`, set REVIEW_PENDING.
6. Status flow: IN_PROGRESS while working -> REVIEW_PENDING when ready for human review. CI_POLLING belongs to the MASTER: set it yourself ONLY when an instruction tells you to, and then the message MUST carry the review request link. NEVER park in CI_POLLING waiting for a human — nothing polls it on your behalf, so a question ENDS the round instead (rule 1).
...
9. Everything you write for a human — status messages, commit messages, the review request, review replies, code comments — follows "How you write" below. It is not advice; text that ignores it is rework for the human who reads it.
