...
| "How long has this request been hanging?" | — | The `MR 8h` chip on the card (`request open 8h` in the console): the HOST's own creation time, stamped by every review read. It survives rounds, respawns and restarts, unlike the status clock. |
| No age on a card with a request open | — | No read has said yet — the clock appears after the first sweep or auto-poll. With no `code-host` configured it never does: the paid model read cannot answer it, and jagt will not substitute its own stamp (a resumed request would then read hours old instead of days). |
| "What is `active` for?" | — | Liveness for the watchdog: ANY MCP call from the agent bumps it, keep-alives included. It says the session is breathing, not that anything moved — a status change is the status clock, and what the poller is about to do is the `auto-review` line. |
