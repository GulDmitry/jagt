...
| An agent stops mid-work to ask | — | Its `awaiting: …` report turns the card over to YOU without leaving IN_PROGRESS: the line reads NEEDS INPUT, the move line says to answer it in the agent's window, and one desktop ping titled `needs input` arrives the first time it asks. A question kept the AGENT badge until 2026-08-19, which the needs-my-action filter and count both dropped. |
...
| "What does this status actually mean?" | — | It says itself: `out for review`, `not shipped`, `review passed`. One spelling for both surfaces (`TaskStatus.label()`), with the enum name still on the wire, in `state.json`, and in the chip's tooltip. A status names a STATE — what to do next is the highlighted button. |
...
| A round answered every comment and changed no code | — | The card does NOT ask for you: it reads as waiting on the code host, with `nothing to ship; the open threads are the reviewer's to close`. Shipping it commits nothing and hands the same threads back to the poll. |
| The poll expected on a CI_POLLING round has stopped (window elapsed, off for this task, no round stamp) | `sweep <task>` | The card DOES ask for you — owner flips to you and the hint says so, because otherwise the task reads as waited-on while nothing looks at it. An install with `autoReview.enabled=false` is NOT this case: it polls nothing by configuration, says so once per surface, and its cards stay as they were. |
...
| A task sits at REVERTED and asks for action | `focus` then `ship <task>`, or `done <task>` | Nothing else acts on it, hence YOU. `deploy` is not offered: the revert ADDED a commit, so re-merging the same branch brings nothing — the branch and its commits survive, so the move is new commits and another round, or close it. |
