...
| "What exactly will this revert take out?" | the board's Revert button | The confirmation names the branches it would push a revert commit to and says the scope: the LAST deploy only. Every deploy overwrites the recorded merge commit, so earlier rounds are not in it. |
