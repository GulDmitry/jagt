...
| "Did the checks pass?" | The dot beside that chip, and nothing else: filled red failed, filled green passed, a hollow pulsing ring still going, and no dot means no verdict anything here can read — hover the request for the word the host used. Two elements, because a red run on an APPROVED request is not the same move as a red run on one nobody has looked at |
...
| A line under a card repeating the request link | There is none: the request link and the dot beside it ARE the round. What is left on that line is news only — NEEDS INPUT, ANSWERED, PROBLEM, NEEDS YOU |
