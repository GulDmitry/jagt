...
| The BASE repository still has the branch checked out | nothing | Freed automatically: it is DETACHED at the base branch (never switched to a local one — a per-task base may have no local branch), and a WARN names what it was on. The branch itself moves to the task's worktree, which is where you work on it now. |
...
| "Is anything actually polling?" | — | Both surfaces answer without being asked: the console's dashboard header and the board's chip carry `auto-review on · every 10–60m over 24h` (or `auto-review off`), and every task out for review carries its own `next poll in 4m`. The stamp is absolute, so the countdown stays right between fetches. |
